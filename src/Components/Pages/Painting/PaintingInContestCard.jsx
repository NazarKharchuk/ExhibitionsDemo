...
const PaintingInContestCard = (props) => {
    const { applicationId, votesCount, isConfirmed, isWon, painting } = props.application;
    const { paintingId, name, painter, likesCount, isLiked, ratingCount, avgRating,
        contestVictoriesCount, width, height, imagePath } = painting;
    ...
    const handleVoteClick = async () => {
        const addVote = async (applicationId, profileId) => {
            try {
                dispatch(setLoading({ isLoading: true }));
                const res = await contestApplicationAPI.addVote(applicationId, profileId);
        ...
        if (myProfileId !== null) {
            await addVote(applicationId, myProfileId);
            props.setNeedRefetch(Date.now());
        }
    };

    const handleUnvoteClick = async () => {
        const addUnvote = async (applicationId, profileId) => {
            try {
                dispatch(setLoading({ isLoading: true }));
                const res = await contestApplicationAPI.deleteVote(applicationId, profileId);
        ...
        if (myProfileId !== null) {
            await addUnvote(applicationId, myProfileId);
            props.setNeedRefetch(Date.now());
        }
    };
